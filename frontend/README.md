...
2.  **Run the Application with Docker Compose:**
    ...
    b.  **First-time setup or after code changes (Python/Dependencies):**
        Use this command to build the Docker image and start the services. The `--build` flag is crucial for the initial setup or whenever you've changed Python code (`.py` files), `uv.lock`, or the `Dockerfile`.
        ```bash
    ...
    c.  **For subsequent runs (after initial setup or frontend-only changes):**
